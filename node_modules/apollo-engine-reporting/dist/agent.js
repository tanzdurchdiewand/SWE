...
function getEngineGraphVariant(engine, _logger = console) {
    if (engine === false) {
        return;
    }
...
class ReportData {
    constructor(executableSchemaId, graphVariant) {
        this.header = new apollo_engine_reporting_protobuf_1.ReportHeader({
...
class EngineReportingAgent {
    ...
    async addTrace({ trace, queryHash, document, operationName, source, executableSchemaId, logger, }) {
        if (this.stopped) {
            return;
        }
    ...
    async sendAllReports() {
        await Promise.all(Object.keys(this.reportDataByExecutableSchemaId).map(id => this.sendReport(id)));
    }
    async sendReport(executableSchemaId) {
        const reportData = this.getReportData(executableSchemaId);
        const { report } = reportData;
        reportData.reset();
        ...
        const response = await async_retry_1.default(async () => {
            const curResponse = await apollo_server_env_1.fetch(this.tracesEndpointUrl, {
                method: 'POST',
                headers: {
            ...
            if (curResponse.status >= 500 && curResponse.status < 600) {
                throw new Error(`HTTP status ${curResponse.status}, ${(await curResponse.text()) ||
                    '(no body)'}`);
            }
            else {
                return curResponse;
        ...
        if (response.status < 200 || response.status >= 300) {
            throw new Error(`Error sending report to Apollo Engine servers: HTTP status ${response.status}, ${(await response.text()) || '(no body)'}`);
        }
        if (this.options.debugPrintReports) {
    ...
    async getTraceSignature({ queryHash, operationName, document, source, logger, }) {
        if (!document && !source) {
    ...
    async sendAllReportsAndReportErrors() {
        await Promise.all(Object.keys(this.reportDataByExecutableSchemaId).map(executableSchemaId => this.sendReportAndReportErrors(executableSchemaId)));
    }
