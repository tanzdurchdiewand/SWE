...
/**
 * Das Modul besteht aus der Klasse {@linkcode GemaeldeFileRequestHandler}, um die
 ...
 */
export class GemaeldeFileRequestHandler {
    private readonly service = new GemaeldeFileService();
    ...
    upload(req: Request, res: Response) {
        const { id } = req.params;
        logger.debug('GemaeldeFileRequestHandler.upload(): id=%s', id);
        ...
        req.on('data', (chunk: Uint8Array) => {
            const { length } = chunk;
            logger.debug(
                'GemaeldeFileRequestHandler.upload(): data %d',
                length,
            );
            data.push(chunk);
            ...
            .on('aborted', () =>
                logger.debug('GemaeldeFileRequestHandler.upload(): aborted'),
            )
            .on('end', () => {
                logger.debug(
                    'GemaeldeFileRequestHandler.upload(): end %d',
                    totalBytesInBuffer,
                );
    ...
    async download(req: Request, res: Response) {
        const { id } = req.params;
        logger.debug('GemaeldeFileRequestHandler.downloadBinary(): %s', id);
        if (id === undefined) {
            res.sendStatus(HttpStatus.INTERNAL_ERROR);
            return;
        }
        ...
        if (
            findResult instanceof GemaeldeFileServiceError ||
            findResult instanceof GemaeldeNotExists
        ) {
    ...
    private handleDownloadError(
        err: DownloadError | GemaeldeNotExists,
        res: Response,
    ) {
        if (err instanceof GemaeldeNotExists) {
            ...
            logger.debug(
                'GemaeldeFileRequestHandler.handleDownloadError(): msg=%s',
                msg,
            );
